...
enemy = GetCurrentTarget()
 ...
 if CanUseSpell(myHero, _R) == READY then
  ...
  if Config.m.Jump:Value() and findWardSlot() and CanUseSpell(myHero, _W) == READY then
    MoveToXYZ(GetMousePos())
    jump2creep()
    wardjump()
  end
...
function combo(unit)
  ...
  if ValidTarget(unit, GetRange(myHero)+125) and Config.c.W1:Value() and GetCastName(myHero, _W) ~= "BlindMonkWOne" then
    CastSpell(_W)
  end
  if ValidTarget(unit, GetRange(myHero)+125) and Config.c.W2:Value() and GetCastName(myHero, _W) == "BlindMonkWOne" then
    CastSpell(_W)
  end
end
...
function wardjump()
  if GetDistance(GetMousePos(), creep) > 250 or creep == nil then
    wardJump(GetMousePos())
  end
end
...
function GetInsecPos(a, b)
  ...
  local atob = Vector(start - Vector(b.x, b.y, b.z)):normalized() * 325
  local atob = a + atob
...
function Insec()
  ...
  pos1 = Vector(myHero) + Vector(wardEnd - myHero):normalized() * 610
    if GetDistance(myHero, pos1) >= 150 and CanUseSpell(myHero, _W) == READY then
    wardJump(wardEnd)
    wardjumped = true
    end
    if wardjumped == true and CanUseSpell then
