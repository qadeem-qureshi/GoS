...
enemy = GetCurrentTarget()
 ...
 if CanUseSpell(myHero, _R) == READY then
  ...
  if Config.m.Jump:Value() and getWard() and CanUseSpell(myHero, _W) == READY then
    MoveToXYZ(GetMousePos())
    jump2creep()
    wardjump()
  end
...
function wardjump()
  if GetDistance(GetMousePos(), creep) > 250 then
    wardHop(GetMousePos())
  end
end
...
function wardHop(pos)
  ...
  elseif getWard() and CanUseSpell(myHero, getWard()) == READY then
    pos = Vector(pos)
    pos = Vector(myHero) + Vector(pos - myHero):normalized() * 610
